        ...
        <>
            <li className="clearfix" key={`${Date.now()}_${message.id}`}>
                <div key={`div_${Date.now()}_${message.id}`} className={`message-data ${parseInt(user.id) === parseInt(store.user.id) ? 'text-right' : '' }`} >
                    <span key={`span_${Date.now()}_${message.id}`} className="message-data-time">{new Date(message.createdAt).toLocaleString('ru-RU', { weekday: 'short', year: 'numeric', month: 'long', day: '2-digit', hour: 'numeric', minute: 'numeric', second: 'numeric' })}</span>
                    <img key={`img_${Date.now()}_${message.id}`} src="/assets/img/doctor-preview.png" alt="avatar"/>
                </div>
